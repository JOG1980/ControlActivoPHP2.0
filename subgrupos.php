...
if($activar_busqueda_inicial){
	  ...
	  //$sql = "SELECT c_subgrupo.*, c_estado_registro.nombre as estado_registro_nombre FROM c_subgrupo 
	  //LEFT JOIN c_estado_registro ON c_subgrupo.estado_registro_id = c_estado_registro.estado_registro_id";
  ...
  $sql_grupo = "SELECT * FROM c_grupo";
  
  $results_grupo = $connect_db->query( $sql_grupo );
  $records_grupo = [];
  if($results_grupo->num_rows > 0){
    while( $row = $results_grupo->fetch_object() ){
      $records_grupo[] = $row;
    }
		...
		<div class="container-login100">
			...
			<div class="wrap-login100">
				...
				<div class="container-fluid">
				  <div class="row">
				    <div class="col-sm-3">
				       <div class="my_titulo"><h1>Catálogo de Subgrupos</h1></div>
				    </div>
					...
					<table id="my_table_sorter" class="table table-bordered table-striped">
						<thead class="thead-dark"> <!-- add class="thead-light" for a light header -->
							<tr>
								...
								<th class="filter-select filter-exact">Grupo</th>
							...
							if($activar_busqueda_inicial){
								...
								if(count($records)>0){
									...
									foreach($records as $record){
										...
										echo '<tr>';
										echo '<td><input type="checkbox" id="checkbox_reg_'.$record->subgrupo_id.'">&nbsp;&nbsp;' . ($contador++) .'</td>';
										echo '<td>'. $record->subgrupo_id.'</td>';
										echo '<td>'.$record->fecha_registro.'</td>';
										echo '<td><div id="reg_div_id_'.$record->subgrupo_id.'">'.$record->nombre.'</div></td>';
										echo '<td>'.$record->descripcion.'</td>';
										echo '<td grupo_id="'.$record->grupo_id.'">'.$record->grupo_nombre.'</td>';
										echo '<td estado_registro_id="'.$record->estado_registro_id.'"><div>'.$record->estado_registro_nombre.'</div></td>';
										echo '<td>'; 

										//botones de duplicar
										if($activar_funcion_duplicar){
											echo '<button type="button" id="btn_duplicar_cascada_' . $record->subgrupo_id . '" class="btn my_btn btn-light btn_duplicar_cascada" data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="" ' . $enable_component . '>';
											echo '<img class="icons24x24" src="images/icons/32x32/application_cascade.png" />';
											echo '</button>';
											
											echo '<button type="button" id="btn_duplicar_actualizar_' . $record->subgrupo_id . '" class="btn my_btn btn-light btn_duplicar_actualizar" data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="" '.$enable_component.'>';
											echo '<img class="icons24x24" src="images/icons/32x32/application_double.png" />';
											echo '</button>';
										}

										echo '<button type="button" id="btn_editar_' . $record->subgrupo_id . '" class="btn my_btn btn-light btn_editar" data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="" '.$enable_component.'>';
										echo '<img class="icons24x24" src="images/icons/32x32/application_edit.png" />';
										echo '</button>';
										
										echo '<button type="button" id="btn_borrar_' . $record->subgrupo_id . '" class="btn my_btn btn-light btn_borrar" data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="" '.$enable_component.'>';
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
		   ...
		   <div class="form-group">
		  		...
		  		<div class="my_modal_campo_container">
		  			<div class="my_modal_campo_label"><label for="se_grupo_id">Grupo</label></div>
			  		<div class="my_modal_campo"><select class="form-control my_form-control" id="se_grupo_id">
					    	<?php
								if( count($records_grupo) > 0 ){
									$contador = 1;
									echo '<option value=""></option>';
									foreach($records_grupo as $record){
										echo '<option value="'.$record->grupo_id.'">'.$record->nombre.'</option>';
									}
								}
							?>
						</select>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
		   ...
		   <div class="form-group">
		  		...
		  		<div class="my_modal_campo_container">
		  			<div class="my_modal_buscar_cb" style="height: 53px;"><input type="checkbox" id="cb_buscar_campo_grupo"></div>
		  			<div class="my_modal_campo_label"><label for="se_buscar_campo_grupo_id">Grupo</label></div>
			  		<div class="my_modal_campo"><select class="form-control my_form-control" id="se_buscar_campo_grupo_id" disabled>
					    	<?php
								if( count($records_grupo) > 0 ){
									$contador = 1;
									foreach($records_grupo as $record){
      ...
      <div class="modal-body">
		 ...
		 <div class="form-group"  style="padding-left: 20px;">
        	...
        	<div style="display: flex; align-items: center;"><input type="checkbox" id="cb_modal_seleccion_campo_fecha_registro" my_campo_tabla="3"><label>&nbsp;&nbsp;Fecha Registro</label></div>
		  	...
		  	<div style="display: flex; align-items: center;"><input type="checkbox" id="cb_modal_seleccion_campo_grupo" my_campo_tabla="6"><label>&nbsp;&nbsp;Grupo</label></div>
...
<script type="text/javascript" src="js/subgrupos.js"></script>		
