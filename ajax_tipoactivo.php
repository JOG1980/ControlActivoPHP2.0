<?php
  ...
  if($funcion === 'buscar_registros'){
      ...
      LEFT JOIN c_estado_registro ON c_tipo_activo.estado_registro_id = c_estado_registro.estado_registro_id WHERE 1 ";
     ...
     if($estado_registro_id != NULL && $estado_registro_id != ""){ //se valida diferente de vacio, ya que es un campo que no puede ser nulo
      ...
      if( ($fecha_registro_inicial != NULL && $fecha_registro_inicial!="") &&
          ($fecha_registro_final != NULL && $fecha_registro_final!="")) { //se valida diferente de vacio, ya que es un campo que no puede ser nulo
             $sql = $sql . " AND  c_tipo_activo.fecha_registro BETWEEN '".$fecha_registro_inicial." 00:00:00' AND '".$fecha_registro_final." 23:59:59' ";
      }
  ...
  //funcion de nuevo registro ---------------------------------------------------
      ...
      $sql = "INSERT INTO c_tipo_activo (nombre,descripcion,fecha_registro,estado_registro_id) 
   ...
   //funcion de editar registro ---------------------------------------------------
      ...
      //$sql = "UPDATE c_tipo_activo SET fecha_registro = '".  date("Y-m.d H:i:s") ."', nombre = '" . $nombre . "', descripcion = '" . $descripcion . "', estado_registro_id = '". $estado_registro_id ."' WHERE tipo_activo_id='" . $id. "'";
       $sql = "UPDATE c_tipo_activo SET nombre = '" . $nombre . "', descripcion = '" . $descripcion . "', estado_registro_id = '". $estado_registro_id ."' WHERE tipo_activo_id='" . $id. "'";
  ...
  //funcion de borrar registro ---------------------------------------------------
    ...
    for($i =0; $i < count($lista_ids) ; $i++){
        ...
        $sql = "DELETE FROM c_tipo_activo WHERE tipo_activo_id ='" . $lista_ids[ $i ] . "'";
