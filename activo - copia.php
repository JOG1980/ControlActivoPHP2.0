...
if($activar_busqueda_inicial){
  ...
  $sql_tipo_activo = "SELECT * FROM c_estado_registro";
					...
					<table id="my_table_sorter" class="table table-bordered table-striped">
						<thead class="thead-dark"> <!-- add class="thead-light" for a light header -->
							<tr>
								...
								<th class="filter-select filter-exact">En Activo Oficial</th>
								<th class="filter-select filter-exact">En Prestamo</th>
								...
								<th class="sorter-false filter-false">Caracteristicas</th>
							...
							if($activar_busqueda_inicial){
								...
								if(count($records)>0){
									...
									foreach($records as $record){
										...
										echo '<td ubicacion_id="'.$record->usuario_id.'">('.$record->usuario_rpe.') '.$record->usuario_nombres.' '.$record->usuario_apellido_paterno.' '.$record->usuario_apellido_materno.'</td>';
