...
if($activar_busqueda_inicial){
	  ...
	  $sql = "SELECT a_activo.*
	  		...
	  		LEFT JOIN c_estado_registro ON a_activo.estado_registro_id = c_estado_registro.estado_registro_id ";
  ...
  //traer todos los registros del tipo activo ------------------------------------------------------------------------
  $sql_tipo_activo = "SELECT * FROM c_tipo_activo";
  
  $results_tipo_activo = $connect_db->query( $sql_tipo_activo );
  $records_tipo_activo = [];
  if($results_tipo_activo->num_rows > 0){
    while( $row = $results_tipo_activo->fetch_object() ){
      $records_tipo_activo[] = $row;
    }
  }

  //traer todos los registros del tipo activo ------------------------------------------------------------------------
  $sql_centro_trabajo = "SELECT * FROM c_centro_trabajo";
  
  $results_centro_trabajo = $connect_db->query( $sql_centro_trabajo );
  $records_centro_trabajo = [];
  if($results_centro_trabajo->num_rows > 0){
    while( $row = $results_centro_trabajo->fetch_object() ){
      $records_centro_trabajo[] = $row;
    }
  }


  //traer todos los registros del tipo activo ------------------------------------------------------------------------
  $sql_ubicacion = "SELECT * FROM c_ubicacion";
  
  $results_ubicacion = $connect_db->query( $sql_ubicacion );
  $records_ubicacion = [];
  if($results_ubicacion->num_rows > 0){
    while( $row = $results_ubicacion->fetch_object() ){
      $records_ubicacion[] = $row;
    }
  }

  //traer todos los registros del tipo activo ------------------------------------------------------------------------
  $sql_usuario = "SELECT * FROM c_usuario";
  
  $results_usuario = $connect_db->query( $sql_usuario );
  $records_usuario = [];
  if($results_usuario->num_rows > 0){
    while( $row = $results_usuario->fetch_object() ){
      $records_usuario[] = $row;
    }
  }
  ...
  $sql_estado_registro = "SELECT * FROM c_estado_registro";
  
  $results_estado_registro = $connect_db->query( $sql_estado_registro );
  $records_estado_registro = [];
  if($results_estado_registro->num_rows > 0){
    while( $row = $results_estado_registro->fetch_object() ){
      $records_estado_registro[] = $row;
    }
		...
		<div class="container-login100">
			...
			<div class="wrap-login100">
				...
				<div class="container-fluid">
				  <div class="row">
				    <div class="col-sm-3">
				       <div class="my_titulo"><h1>Activo</h1></div>
				    </div>
					...
					<table id="my_table_sorter" class="table table-bordered table-striped">
						<thead class="thead-dark"> <!-- add class="thead-light" for a light header -->
							<tr>
								...
								<th class="filter-select filter-exact">Tipo Activo</th>
								<th class="filter-match">Marca</th>
								<th class="filter-match">Modelo</th>
								<th class="filter-match">Número de Serie</th>
								<th class="filter-match">Número de Activo</th>
								<th class="filter-match">Número de Inventario</th>
								<th class="filter-select filter-exact">Centro de Trabajo</th>
								<th class="filter-select filter-exact">Ubicacion</th>
								<th class="filter-select filter-exact">Usuario</th>
								<th class="filter-select filter-exact">TIC</th>
								<th class="filter-select filter-exact">En Uso</th>
								<th class="filter-select filter-exact">Act. Oficial</th>
								<th class="filter-select filter-exact">SIITIC</th>
								<th class="filter-select filter-exact">Prestamo</th>
								<th class="filter-match">Fecha Ingreso</th>
								<th class="filter-match">Fecha Alta</th>
								<th class="filter-match">Fecha Capitalización</th>
								<th class="filter-match">Fecha Obsoleto</th>
								<th class="filter-match">Fecha Baja</th>
								<th class="filter-match">Fecha Salida</th>
								<th class="filter-match">Costo (Pesos)</th>
								<th class="filter-match">Costo (Dolares)</th>
								<th class="filter-select filter-exact">Estado Registro</th>
								<th class="sorter-false filter-false">Más Campos</th>
						...
						<tfoot>
							<tr>
								<th colspan=<?php echo '"'.$numero_columnas.'"'; ?> class="ts-pager">
							...
							if($activar_busqueda_inicial){
								...
								if(count($records)>0){
									...
									foreach($records as $record){
										...
										echo '<tr>';
										echo '<td><input type="checkbox" id="checkbox_reg_'.$record->activo_id.'">&nbsp;&nbsp;'.($contador++).'</td>';
										echo '<td>'. $record->activo_id.'</td>';
										echo '<td>'.$record->fecha_registro.'</td>';
										echo '<td><div id="reg_div_id_'.$record->activo_id.'">'.$record->descripcion.'</div></td>';
										echo '<td tipo_activo_id="'.$record->tipo_activo_id.'">'.$record->tipo_activo_nombre.'</td>';
										echo '<td>'.$record->marca.'</td>';
										echo '<td>'.$record->modelo.'</td>';
										echo '<td>'.$record->numero_serie.'</td>';
										echo '<td>'.$record->numero_activo.'</td>';
										echo '<td>'.$record->numero_inventario.'</td>';
										echo '<td centro_trabajo_id="'.$record->centro_trabajo_id.'">'.$record->centro_trabajo_nombre.'</td>';
										echo '<td ubicacion_id="'.$record->ubicacion_id.'">'.$record->ubicacion_nombre.'</td>';
										echo '<td usuario_id="'.$record->usuario_id.'">('.$record->usuario_rpe.') '.$record->usuario_nombres.' '.$record->usuario_apellido_paterno.' '.$record->usuario_apellido_materno.'</td>';
										echo '<td>'.$record->es_tic.'</td>';
										echo '<td>'.$record->en_uso.'</td>';
										echo '<td>'.$record->en_activo_oficial.'</td>';
										echo '<td>'.$record->en_siitic.'</td>';
										echo '<td>'.$record->en_prestamo.'</td>';
										echo '<td>'.$record->fecha_ingreso.'</td>';
										echo '<td>'.$record->fecha_alta_activo.'</td>';
										echo '<td>'.$record->fecha_capitalizacion.'</td>';

										echo '<td>'.$record->fecha_obsoleto.'</td>';
										echo '<td>'.$record->fecha_baja_activo.'</td>';
										echo '<td>'.$record->fecha_salida.'</td>';

										echo '<td>'.$record->costo_pesos.'</td>';
										echo '<td>'.$record->costo_dolares.'</td>';
										
										echo '<td estado_registro_id="'.$record->estado_registro_id.'"><div>'.$record->estado_registro_nombre.'</div></td>';
									

										//botones de caracteristicas
										echo '<td>'; 
											echo '<button type="button" id="btn_ver_grupos_' . $record->activo_id . '" class="btn my_btn btn-light btn_ver_grupos" data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="" ' . $enable_component . '>';
											echo '<img class="icons24x24" src="images/icons/32x32/category_item_select.png" />';
											echo '</button>';
											
											echo '<button type="button" id="btn_ver_comentarios_' . $record->activo_id . '" class="btn my_btn btn-light btn_ver_comentarios" data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="" '.$enable_component.'>';
											echo '<img class="icons24x24" src="images/icons/32x32/comments.png" />';
											echo '</button>';
										echo '</td>'; 
										...
										if($activar_funcion_duplicar){
											echo '<button type="button" id="btn_duplicar_cascada_' . $record->activo_id . '" class="btn my_btn btn-light btn_duplicar_cascada" data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="" ' . $enable_component . '>';
											echo '<img class="icons24x24" src="images/icons/32x32/application_cascade.png" />';
											echo '</button>';
											
											echo '<button type="button" id="btn_duplicar_actualizar_' . $record->activo_id . '" class="btn my_btn btn-light btn_duplicar_actualizar" data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="" '.$enable_component.'>';
											echo '<img class="icons24x24" src="images/icons/32x32/application_double.png" />';
											echo '</button>';
										}

										echo '<button type="button" id="btn_editar_' . $record->activo_id . '" class="btn my_btn btn-light btn_editar" data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="" '.$enable_component.'>';
										echo '<img class="icons24x24" src="images/icons/32x32/application_edit.png" />';
										echo '</button>';
										
										echo '<button type="button" id="btn_borrar_' . $record->activo_id . '" class="btn my_btn btn-light btn_borrar" data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="" '.$enable_component.'>';
 		   ...
 		   <div class="form-group">
		  		...
		  		<div class="row">
				    ...
				    <div class="col-sm-4"> 
					  	 <div class="my_modal_campo_container">
				  			...
				  			<div class="my_modal_campo_cactivo"><input type="text" class="form-control" style="width: 100px;" id="tb_id" name="tb_id" value="" disabled></div>
				  		</div>

				    </div>
				    <div class="col-sm-5">
				    	<div class="my_modal_campo_container">
				  			<div class="my_modal_campo_label"><label for="tb_fecha_registro_id" disabled>Fecha Registro</label></div>
				  			<div class="my_modal_campo_cactivo"><input type="text" class="form-control my_form-control" id="tb_fecha_registro_id" value=""></div>
				  		</div>
			  ...
			  <div class="row">
			    <div class="col-sm">
			  		...
			  		<div class="my_modal_group">
				  		<div class="my_modal_campo_container" style="height: 90px; ">
				  			<div class="my_modal_campo_label" style="height: 79px;"><label for="ta_descripcion_id">Descripción</label></div>
				  			<div class="my_modal_campo_cactivo"><textarea class="form-control" id="ta_descripcion_id" rows="3"></textarea></div>
				  		</div>
				  	</div><!--end group-->

				  	<div class="my_modal_group"> <!------------------------------------------------------------------------------------>
				  		<div class="my_modal_campo_container">
				  			<div class="my_modal_campo_label"><label for="se_tipo_activo_id">Tipo Activo</label></div>
					  		<div class="my_modal_campo_cactivo">
					  			<select class="form-control my_form-control" id="se_tipo_activo_id">
					  				<option value="">&nbsp;</option>
							    	<?php
										if( count($records_tipo_activo) > 0 ){
											$contador = 1;
											foreach($records_tipo_activo as $record){
												echo '<option value="'.$record->tipo_activo_id.'">'.$record->nombre.'</option>';
											}
										}
									?>
								</select>
				  		...
				  		<div class="my_modal_campo_container">
				  			<div class="my_modal_campo_label"><label for="tb_marca_id">Marca</div>
				  			<div class="my_modal_campo_cactivo"><input type="text" class="form-control my_form-control" id="tb_marca_id" name="tb_marca" value=""></div>
				  		</div>

				  		<div class="my_modal_campo_container">
				  			<div class="my_modal_campo_label"><label for="tb_modelo_id">Modelo</div>
				  			<div class="my_modal_campo_cactivo"><input type="text" class="form-control my_form-control" id="tb_modelo_id" name="tb_modelo" value=""></div>
				  		</div>

				  		<div class="my_modal_campo_container">
				  			<div class="my_modal_campo_label"><label for="tb_numero_serie_id">Número de Serie</div>
				  			<div class="my_modal_campo_cactivo"><input type="text" class="form-control my_form-control" id="tb_numero_serie_id" name="tb_numero_serie" value=""></div>
				  		</div>
					...
					<div class="my_modal_group"> <!------------------------------------------------------------------------------------>
				  		<div class="my_modal_campo_container">
				  			<div class="my_modal_campo_label"><label for="tb_numero_activo_id">Número de Activo</div>
				  			<div class="my_modal_campo_cactivo"><input type="text" class="form-control my_form-control" id="tb_numero_activo_id" name="tb_numero_activo" value=""></div>
				  		</div>

				  		<div class="my_modal_campo_container">
				  			<div class="my_modal_campo_label"><label for="tb_numero_inventario_id">Número de Inventario</div>
				  			<div class="my_modal_campo_cactivo"><input type="text" class="form-control my_form-control" id="tb_numero_inventario_id" name="tb_numero_inventario" value=""></div>
				  		</div>
				  	</div><!--end group-->

				  	<div class="my_modal_group"> <!------------------------------------------------------------------------------------>
				  		<div class="my_modal_campo_container">
				  			<div class="my_modal_campo_label"><label for="se_centro_trabajo_id">Centro de Trabajo</label></div>
					  		<div class="my_modal_campo_cactivo">
					  			<select class="form-control" id="se_centro_trabajo_id">
					  				<option value="">&nbsp;</option>
							    	<?php
										if( count($records_centro_trabajo) > 0 ){
											$contador = 1;
											foreach($records_centro_trabajo as $record){
												echo '<option value="'.$record->centro_trabajo_id.'">'.$record->nombre.'</option>';
											}
										}
									?>
								</select>
						...
						<div class="my_modal_campo_container">
				  			<div class="my_modal_campo_label"><label for="se_ubicacion_id">Ubicación</label></div>
					  		<div class="my_modal_campo_cactivo">
					  			<select class="form-control my_form-control" id="se_ubicacion_id">
					  				<option value="">&nbsp;</option>
							    	<?php
										if( count($records_ubicacion) > 0 ){
											$contador = 1;
											foreach($records_ubicacion as $record){
												echo '<option value="'.$record->ubicacion_id.'">'.$record->nombre.'</option>';
											}
										}
									?>
								</select>
						...
						<div class="my_modal_campo_container">
				  			<div class="my_modal_campo_label"><label for="se_usuario_id">Usuario</label></div>
					  		<div class="my_modal_campo_cactivo">
					  			<select class="form-control my_form-control" id="se_usuario_id">
					  				<option value="">&nbsp;</option>
							    	<?php
										if( count($records_usuario) > 0 ){
											$contador = 1;
											foreach($records_usuario as $record){
												echo '<option value="'.$record->usuario_id.'">('.$record->rpe.') '.$record->nombres.' '.$record->apellido_paterno.' '.$record->apellido_materno.'</option>';
											}
										}
									?>
								</select>
			    ...
			    <div class="col-sm" style="border: 1px solid red;  border-radius: 25px;"><!--columna 2-->

			  		<div class="my_modal_campo_container">
			  			<div class="my_modal_campo_label"><label for="tb_fecha_ingreso_id" disabled>Fecha Ingreso</label></div>
			  			<div class="my_modal_campo_cactivo"><input type="text" class="form-control my_form-control" id="tb_fecha_ingreso_id" value=""></div>
			  		</div>

			  		<div class="my_modal_campo_container">
			  			<div class="my_modal_campo_label"><label for="tb_fecha_alta_id" disabled>Fecha Alta</label></div>
			  			<div class="my_modal_campo_cactivo"><input type="text" class="form-control my_form-control" id="tb_fecha_alta_id" value=""></div>
			  		</div>

			  		<div class="my_modal_campo_container">
			  			<div class="my_modal_campo_label"><label for="tb_fecha_baja_id" disabled>Fecha Baja</label></div>
			  			<div class="my_modal_campo_cactivo"><input type="text" class="form-control my_form-control" id="tb_fecha_baja_id" value=""></div>
			  		</div>

			  		<div class="form-check">
			  			<hr>
			  			<div style="display: inline-block; width: 120px;">
						    <input class="form-check-input" type="checkbox" value="" id="cb_en_uso_id">
						    <label class="form-check-label" for="cb_en_uso_id">En uso</label>
						</div>
						<div  style="display: inline-block; width: 190px;">
						    <input class="form-check-input" type="checkbox" value="" id="cb_en_activo_oficial_id">
						    <label class="form-check-label" for="cb_en_activo_oficial_id">En Activo Oficial</label>
						</div>
						<hr>
					</div>
					...
					<div class="form-check">
			  			<hr>
			  			<div style="display: inline-block; width: 120px;">
						  	<input class="form-check-input" type="checkbox" value="" id="cb_en_prestamo_id">
						  	<label class="form-check-label" for="cb_en_prestamo_id">En prestamo</label>
						</div>
						<hr>
					</div>

					<div class="my_modal_campo_container">
			  			<div class="my_modal_campo_label"><label for="se_estado_registro_id">Estado del registro</label></div>
				  		<div class="my_modal_campo_cactivo"><select class="form-control my_form-control" id="se_estado_registro_id">
						    	<?php
									if( count($records_estado_registro) > 0 ){
										$contador = 1;
										foreach($records_estado_registro as $record){
											echo '<option value="'.$record->estado_registro_id.'">'.$record->nombre.'</option>';
										}
									}
								?>
							</select>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
		   ...
		   <div class="form-group">
		  		...
		  		<div class="my_modal_campo_container">
		  			<div class="my_modal_buscar_cb" style="height: 53px;"><input type="checkbox" id="cb_buscar_campo_estado_registro"></div>
		  			<div class="my_modal_campo_label"><label for="se_buscar_campo_estado_registro_id">Estado del registro</label></div>
			  		<div class="my_modal_campo"><select class="form-control my_form-control" id="se_buscar_campo_estado_registro_id" disabled>
					    	<?php
								if( count($records_estado_registro) > 0 ){
									$contador = 1;
									foreach($records_estado_registro as $record){
      ...
      <div class="modal-body">
		 ...
		 <div class="form-group"  style="padding-left: 20px;">
        	...
        	<div style="display: flex; align-items: center;"><input type="checkbox" id="cb_modal_seleccion_campo_fecha_registro" my_campo_tabla="3"><label>&nbsp;&nbsp;Fecha Registro</label></div>
		  	...
		  	<div style="display: flex; align-items: center;"><input type="checkbox" id="cb_modal_seleccion_campo_estado_registro" my_campo_tabla="6"><label>&nbsp;&nbsp;Estado Registro </label></div>
      	</div>
      	</div>
      	<div class="modal-footer">
        	<button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button>
