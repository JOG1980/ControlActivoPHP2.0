<?php
     ...
     if($estado_registro_id != NULL && $estado_registro_id != ""){ //se valida diferente de vacio, ya que es un campo que no puede ser nulo
      ...
      if( ($fecha_registro_inicial != NULL && $fecha_registro_inicial!="") &&
          ($fecha_registro_final != NULL && $fecha_registro_final!="")) { //se valida diferente de vacio, ya que es un campo que no puede ser nulo
             $sql = $sql . " AND  c_grupo.fecha_registro BETWEEN '".$fecha_registro_inicial." 00:00:00' AND '".$fecha_registro_final." 23:59:59' ";
      }
  ...
  //funcion de nuevo registro ---------------------------------------------------
      ...
      $sql = "INSERT INTO c_grupo (nombre,descripcion,fecha_registro,estado_registro_id) 
   ...
   //funcion de editar registro ---------------------------------------------------
      ...
      //$sql = "UPDATE c_grupo SET fecha_registro = '".  date("Y-m.d H:i:s") ."', nombre = '" . $nombre . "', descripcion = '" . $descripcion . "', estado_registro_id = '". $estado_registro_id ."' WHERE grupo_id='" . $id. "'";
       $sql = "UPDATE c_grupo SET nombre = '" . $nombre . "', descripcion = '" . $descripcion . "', estado_registro_id = '". $estado_registro_id ."' WHERE grupo_id='" . $id. "'";
  ...
  //funcion de borrar registro ---------------------------------------------------
    ...
    for($i =0; $i < count($lista_ids) ; $i++){
        ...
        $sql = "DELETE FROM c_grupo WHERE grupo_id ='" . $lista_ids[ $i ] . "'";
