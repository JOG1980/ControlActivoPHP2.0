$(function() {
	...
	var numero_filtros_tabla = 5; //se usa cuando se agregan popovers a los filtros, en la funcion pintarPopoversSobreFiltrosTabla()

	var columna_estado_registros = 2; //es para saber en que columa estan mostrados el estado d¿registro, para poder apuntar y tomar ese valoir para pintar colores.
	var columna_botones_acciones = 6; //es para saber en que columa estan mostrados los botones de las acciones.

	//esta es la lista del estado en el que se muestran los campos
	//                                       index,    id, fecha, nombre, descripcion
	//                       posiciones        0      1       2     3          4            5  
	var lista_campos_mostrados_default     = [true, false, false,   true,        true]; //esta tabla no cambia es la default
	var lista_campos_mostrados 		       = [true, false, false,   true,        true];
	var lista_campos_mostrados_habilitados = [true,  true,  true,   true,        true]; //los campos con true solo son mostrados al perfil del admin
	...
	var busqueda_parametros = [null,  null,       null,         null,       null];
		...
		$("#my_table_sorter").tablesorter({
			...
			widgetOptions : {
				...
				//resizable_addLastColumn : true,
			     ...
			     resizable_widths : [ '100px','100px', '160px', , , '158px'],
	...
	var updateTableParameters = function(){
		...
		if(!activar_funcion_duplicar){
			...
			$("#my_table_sorter")[0].config.widgetOptions.resizable_widths = [ '100px','100px', '160px', ,, '90px' ];
	...
	var ejecutarBuscarRegistrosAjax = function(id, nombre, descripcion, fecha_registro_inicial, fecha_registro_final){
    	...
    	$.ajax({
            url : 'ajax_estadoregistros.php',    // la URL para la petición
    	    ...
    	    success: function (data, status, xhr) {
    	    	...
    	    	for(var i=0 ; i<records.length ; i++){
						...
						contenido += "<tr>";
    	    			contenido += '<td><input type="checkbox" id="checkbox_reg_'  + estado_registro_id + '">&nbsp;&nbsp;' + (i+1) + '</td>';
    	    			contenido += '<td>' + estado_registro_id + '</td>';
    	    			contenido += '<td>' + fecha_registro + '</td>';
    	    			contenido += '<td><div id="reg_div_id_' + estado_registro_id + '">' + nombre +'</div></td>';
    	    			contenido += '<td>' + descripcion + '</td>';
    	    			contenido += '<td>';

    	    			//funciones de duplicar ------------
    	    			if(activar_funcion_duplicar){
	    	    			contenido += '<button type="button" id="btn_duplicar_cascada_' + estado_registro_id + '" class="btn my_btn btn-light btn_duplicar_cascada" \
							...
							contenido += '</button>';
							contenido += '<button type="button" id="btn_duplicar_actualizar_' + estado_registro_id + '" class="btn my_btn btn-light btn_duplicar_actualizar" \
	    	    			               data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="">';
							contenido += '<img class="icons24x24" src="images/icons/32x32/application_double.png" />';
							contenido += '</button>';
						}//end botones duplicar

						contenido += '<button type="button" id="btn_editar_' + estado_registro_id + '" class="btn my_btn btn-light btn_editar" \
						               data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="" />';
						contenido += '<img class="icons24x24" src="images/icons/32x32/application_edit.png" />';
						contenido += '</button>';
						contenido += '<button type="button" id="btn_borrar_' + estado_registro_id + '" class="btn my_btn btn-light btn_borrar" \
						               data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="" />';
	...
	var cargarUltimaBusquedaTableSorterAjax = function(){
    	...
    	ejecutarBuscarRegistrosAjax( id, nombre, descripcion, fecha_registro_inicial, fecha_registro_final);
	...
	var ejecutarCrearEditarRegistroAjax = function(tipo_funcion, id, nombre, descripcion){
		var obj = new Object();
		obj.id = id;
		obj.nombre = nombre;
		obj.descripcion = descripcion;
	...
	var definirBotonVerRegistroVerVentanaModal = function(){
		$( "[id^='reg_div_id_']").click(function() {
			...
			$("#modal_nuevo_edit_duplicar_borrar .modal-title").html("Cat. de Estado de Registros - Ver Registro");//titulo de la ventana
	...
	var definirBotonDuplicarCascada = function(){
		...
		$( ".btn_duplicar_cascada" ).click(function() {
		   ...
		   if(validarCampos(nombre)){
			    ...
			    ejecutarCrearEditarRegistroAjax("nuevo_registro", null, nombre, descripcion);
			}
		});
	}
	...
	var definirBotonDuplicarActualizar = function(){
		...
		$( ".btn_duplicar_actualizar" ).click(function() {
		    ...
		    //alert(renglon_estado_registro_id);
			$("#modal_nuevo_edit_duplicar_borrar .modal-title").html("Cat. de Estado de Registros - Dupicar Registro");//titulo de la ventana
	...
	var definirBotonEditarMostrarVentanaModal = function(){
		$( ".btn_editar" ).click(function() {
		    ...
		    //alert(renglon_estado_registro_id);
			$("#modal_nuevo_edit_duplicar_borrar .modal-title").html("Cat. de Estado de Registros - Editar Registro");//titulo de la ventana
...
//boton borrar -----------------------------------------
	var definirBotonBorrarMostrarVentanaModal = function(){	
		...
		$( ".btn_borrar" ).click(function() {
		    ...
		    //alert(renglon_estado_registro_id);
			$("#modal_nuevo_edit_duplicar_borrar .modal-title").html("Cat. de Estado de Registros - Borrar Registro");//titulo de la ventana
	...
	$( "#btn_buscar_base_datos_todos" ).click(function() {
		...
		ejecutarBuscarRegistrosAjax( null, null, null, null, null);
	});

	//boton buscar -----------------------------------------	
	$( "#btn_buscar_base_datos" ).click(function() {
					
			$("#modal_buscar .modal-title").html("Cat. de Cat. de Estado de Registros - Buscar Registro");//titulo de la ventana
	...
	$( "#btn_mostrar_modal_nuevo_registro" ).click(function() {
			...
			$("#modal_nuevo_edit_duplicar_borrar .modal-title").html("Cat. de Estado de Registros - Nuevo Registro");//titulo de la ventana
	...
	$( "#btn_mostrar_modal_borrar_bloque_registro" ).click(function() {
			...
			$("#modal_borrar .modal-title").html("Cat. de Estado de Registros - Borar Registros Seleccionados");//titulo de la ventana
	...
	$( "#btn_editar_ejecutar" ).click(function() {
		   ...
		   if(validarCampos(nombre)){
		   		//realizamos la creacion o modificacion del registro
				ejecutarCrearEditarRegistroAjax("editar_registro", renglon_id, nombre, descripcion);
			}
	...
	var ajustarEstadoRegistros = function(){
		  ...
		  $("#my_table_sorter tr").each(function(){
			    ...
			    var estado_registro_id = $(this).find("td:nth-child("+columna_estado_registros+")").text();
