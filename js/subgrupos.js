$(function() {
	...
	//                                       index,    id, fecha, nombre, grupo,descripcion, estado_registro
	...
	//Parametros:                id,nombre, grupo, descripcion,fecha_inicial,fecha_final,estado_registro 
	...
	var ejecutarBuscarRegistrosAjax = function(id, nombre, descripcion, fecha_registro_inicial, fecha_registro_final, grupo_id, estado_registro_id){
    	...
    	$.ajax({
            url : 'ajax_subgrupos.php',    // la URL para la petición
    	    ...
    	    success: function (data, status, xhr) {
    	    	...
    	    	for(var i=0 ; i<records.length ; i++){
    	    			...
    	    			var subgrupo_id = records[i].subgrupo_id;
						var nombre         = records[i].nombre;
						var descripcion    = records[i].descripcion;
						var fecha_registro = records[i].fecha_registro;
						var grupo_id = records[i].grupo_id;
						var grupo_nombre = records[i].grupo_nombre;
						...
						contenido += "<tr>";
    	    			contenido += '<td><input type="checkbox" id="checkbox_reg_'  + subgrupo_id + '">&nbsp;&nbsp;' + (i+1) + '</td>';
    	    			contenido += '<td>' + subgrupo_id + '</td>';
    	    			contenido += '<td>' + fecha_registro + '</td>';
    	    			contenido += '<td><div id="reg_div_id_' + subgrupo_id + '">' + nombre +'</div></td>';
    	    			contenido += '<td>' + descripcion + '</td>';
    	    			contenido += '<td grupo_id="' + grupo_id + '">' + grupo_nombre +'</td>';
    	    			contenido += '<td estado_registro_id="' + estado_registro_id + '"><div>' + estado_registro_nombre + '</div></td>';
    	    			contenido += '<td>';

    	    			//funciones de duplicar ------------
    	    			if(activar_funcion_duplicar){
	    	    			contenido += '<button type="button" id="btn_duplicar_cascada_' + subgrupo_id + '" class="btn my_btn btn-light btn_duplicar_cascada" \
							...
							contenido += '</button>';
							contenido += '<button type="button" id="btn_duplicar_actualizar_' + subgrupo_id + '" class="btn my_btn btn-light btn_duplicar_actualizar" \
	    	    			               data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="">';
							contenido += '<img class="icons24x24" src="images/icons/32x32/application_double.png" />';
							contenido += '</button>';
						}//end botones duplicar

						contenido += '<button type="button" id="btn_editar_' + subgrupo_id + '" class="btn my_btn btn-light btn_editar" \
						               data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="" />';
						contenido += '<img class="icons24x24" src="images/icons/32x32/application_edit.png" />';
						contenido += '</button>';
						contenido += '<button type="button" id="btn_borrar_' + subgrupo_id + '" class="btn my_btn btn-light btn_borrar" \
						               data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="" />';
	...
	var cargarUltimaBusquedaTableSorterAjax = function(){
    	...
    	var grupo_id               = busqueda_parametros[5];
    	var estado_registro_id     = busqueda_parametros[6]; 
    	 
    	ejecutarBuscarRegistrosAjax( id, nombre, descripcion, fecha_registro_inicial, fecha_registro_final, grupo_id, estado_registro_id);
	...
	var ejecutarCrearEditarRegistroAjax = function(tipo_funcion, id, nombre, descripcion, grupo_id, estado_registro_id){
		var obj = new Object();
		obj.id = id;
		obj.nombre = nombre;
		obj.descripcion = descripcion;
	...
	var definirBotonVerRegistroVerVentanaModal = function(){
		$( "[id^='reg_div_id_']").click(function() {
		    ...
		    var renglon_grupo_id = $(this).parent().parent().find("td:nth-child(6)").attr("grupo_id");
			...
			$("#modal_nuevo_edit_duplicar_borrar .modal-title").html("Catálogo de Subgrupos - Ver Registro");//titulo de la ventana
			...
			$('#se_grupo_id').val(renglon_grupo_id); //1 nuevo. inicializamos el valor
			...
			$('#se_grupo_id').prop("disabled", true);  //valor del centro de trabajo
	...
	var definirBotonDuplicarCascada = function(){
		...
		$( ".btn_duplicar_cascada" ).click(function() {
		    ...
		    var grupo_id = $(this).parent().parent().find("td:nth-child(6)").attr("grupo_id");
		   ...
		   if(validarCampos(nombre,grupo_id)){
		   		//realizamos la creacion o modificacion del registro
			    ejecutarCrearEditarRegistroAjax("nuevo_registro", null, nombre, descripcion, grupo_id, estado_registro_id);
			}
		});
	}
	...
	var definirBotonDuplicarActualizar = function(){
		...
		$( ".btn_duplicar_actualizar" ).click(function() {
		    ...
		    //alert(renglon_estado_registro_id);
			$("#modal_nuevo_edit_duplicar_borrar .modal-title").html("Catálogo de Subgrupos - Dupicar Registro");//titulo de la ventana
			...
			$('#se_grupo_id').prop("disabled", false);  //valor del centro de trabajo
	...
	var definirBotonEditarMostrarVentanaModal = function(){
		$( ".btn_editar" ).click(function() {
		    ...
		    //alert(renglon_estado_registro_id);
			$("#modal_nuevo_edit_duplicar_borrar .modal-title").html("Catálogo de Subgrupos - Editar Registro");//titulo de la ventana
...
//boton borrar -----------------------------------------
	var definirBotonBorrarMostrarVentanaModal = function(){	
		...
		$( ".btn_borrar" ).click(function() {
		    ...
		    //alert(renglon_estado_registro_id);
			$("#modal_nuevo_edit_duplicar_borrar .modal-title").html("Catálogo de Subgrupos - Borrar Registro");//titulo de la ventana
	...
	$( "#btn_buscar_base_datos" ).click(function() {
			...
			$("#modal_buscar .modal-title").html("Catálogo de Subgrupos - Buscar Registro");//titulo de la ventana
	...
	$( "#btn_mostrar_modal_nuevo_registro" ).click(function() {
			...
			$("#modal_nuevo_edit_duplicar_borrar .modal-title").html("Catálogo de Subgrupos - Nuevo Registro");//titulo de la ventana
			...
			$('#se_grupo_id').val(grupo_id_default); //inicializamos el valor
	...
	$( "#btn_mostrar_modal_borrar_bloque_registro" ).click(function() {
			...
			$("#modal_borrar .modal-title").html("Catálogo de Subgrupos - Borar Registros Seleccionados");//titulo de la ventana
	...
	$( "#btn_nuevo_ejecutar" ).click(function() {
			...
			var grupo_id = $('#se_grupo_id').val(); //1 nuevo
	...
	$( "#btn_editar_ejecutar" ).click(function() {
		   ...
		   if(validarCampos(nombre,grupo_id)){
		   		//realizamos la creacion o modificacion del registro
				ejecutarCrearEditarRegistroAjax("editar_registro", renglon_id, nombre, descripcion, grupo_id, estado_registro_id);
			}
	...
	$( "#btn_buscar_ejecutar" ).click(function() {
		...
		var grupo_id = $('#se_buscar_campo_grupo_id').val().trim(); //1 nuevo
		...
		var cb_buscar_campo_grupo_is_click = $('#cb_buscar_campo_grupo').is(":checked");
		...
		if(!cb_buscar_campo_grupo_is_click){
			grupo_id=null;
		}
    	...
    	busqueda_parametros[5] = grupo_id;
	...
	var validarCampos = function(nombre, grupo_id){
		...
		else if(grupo_id == null || grupo_id == ""){
			//colocamos el texto del aviso en la modal ------	
			$("#modal_label_msg").html("El campo grupo no puede ser vacío.");
			//etiqueta de mensaje de la ventana modal ----
			$('#modal_label_msg').parent().show();
			return false;
		}
	...
	var initElements = function(){
		...
		$(".my_titulo").css("color", color_distintivo_subgrupos_texto);
		$(".my_titulo").css("background-color", color_distintivo_subgrupos_fondo); //asignamos el color de la barra distintiva en las modales

		//barra distintiva en ventanas -----------------------------
		$(".barra_distintiva_color").css("background-color", color_distintivo_subgrupos_fondo); //asignamos el color de la barra distintiva en las modales
	}
