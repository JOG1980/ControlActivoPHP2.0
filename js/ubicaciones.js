$(function() {
	...
	var numero_filtros_tabla = 7; //se usa cuando se agregan popovers a los filtros, en la funcion pintarPopoversSobreFiltrosTabla()

	var columna_estado_registros = 7; //es para saber en que columa estan mostrados el estado d¿registro, para poder apuntar y tomar ese valoir para pintar colores.
	var columna_botones_acciones = 8; //es para saber en que columa estan mostrados los botones de las acciones.

	//esta es la lista del estado en el que se muestran los campos
	//                                       index,    id, fecha, nombre, centro_trabajo,descripcion, estado_registro
	//                       posiciones        0      1       2     3                 4            5           6  
	var lista_campos_mostrados_default     = [true, false, false,   true,           true,       true,           false]; //esta tabla no cambia es la default
	var lista_campos_mostrados 		       = [true, false, false,   true,           true,       true,           false];
	var lista_campos_mostrados_habilitados = [true,  true,  true,   true,           true,       true,            true]; //los campos con true solo son mostrados al perfil del admin
	...
	//Parametros:                id,nombre, centro_trabajo, descripcion,fecha_inicial,fecha_final,estado_registro 
	var busqueda_parametros = [null,  null,           null,        null,         null,       null,           null];

	var retornar_ventana_despues_de_error = false;

	//VARIABLES PROPIAS DEL CATALOGO --------------------------------------
	var centro_trabajo_id_default = 1;
	...
	$("#my_table_sorter").tablesorter({
		...
		widgetOptions : {
			...
			//resizable_addLastColumn : true,
		     ...
		     resizable_widths : [ '100px','100px', '160px', , , ,'140px', '158px'],
			...
			filter_cssFilter: [
				...
				'form-control custom-select', //centro de trabajo la base de datos
	...
	var updateTableParameters = function(){
		...
		if(!activar_funcion_duplicar){
			...
			$("#my_table_sorter")[0].config.widgetOptions.resizable_widths = [ '100px','100px', '160px', , , ,'140px', '90px' ];
	...
	var ejecutarBuscarRegistrosAjax = function(id, nombre, descripcion, fecha_registro_inicial, fecha_registro_final, centro_trabajo_id, estado_registro_id){
    	...
    	$.ajax({
            url : 'ajax_ubicaciones.php',    // la URL para la petición
    	    ...
    	    success: function (data, status, xhr) {
    	    	...
    	    	for(var i=0 ; i<records.length ; i++){
						...
						contenido += "<tr>";
    	    			contenido += '<td><input type="checkbox" id="checkbox_reg_'  + ubicacion_id + '">&nbsp;&nbsp;' + (i+1) + '</td>';
    	    			contenido += '<td>' + ubicacion_id + '</td>';
    	    			contenido += '<td>' + fecha_registro + '</td>';
    	    			contenido += '<td><div id="reg_div_id_' + ubicacion_id + '">' + nombre +'</div></td>';
    	    			...
    	    			if(activar_funcion_duplicar){
	    	    			contenido += '<button type="button" id="btn_duplicar_cascada_' + ubicacion_id + '" class="btn my_btn btn-light btn_duplicar_cascada" \
							...
							contenido += '</button>';
							contenido += '<button type="button" id="btn_duplicar_actualizar_' + ubicacion_id + '" class="btn my_btn btn-light btn_duplicar_actualizar" \
	    	    			               data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="">';
							contenido += '<img class="icons24x24" src="images/icons/32x32/application_double.png" />';
							contenido += '</button>';
						}//end botones duplicar

						contenido += '<button type="button" id="btn_editar_' + ubicacion_id + '" class="btn my_btn btn-light btn_editar" \
						               data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="" />';
						contenido += '<img class="icons24x24" src="images/icons/32x32/application_edit.png" />';
						contenido += '</button>';
						contenido += '<button type="button" id="btn_borrar_' + ubicacion_id + '" class="btn my_btn btn-light btn_borrar" \
						               data-container="body" data-toggle="popover" title=""  data-placement="left" data-content="" />';
	...
	var cargarUltimaBusquedaTableSorterAjax = function(){
    	...
    	var nombre                 = busqueda_parametros[1];
    	var descripcion            = busqueda_parametros[2];
    	var fecha_registro_inicial = busqueda_parametros[3];
    	var fecha_registro_final   = busqueda_parametros[4];
    	var centro_trabajo_id      = busqueda_parametros[5];
    	var estado_registro_id     = busqueda_parametros[6]; 
    	 
    	ejecutarBuscarRegistrosAjax( id, nombre, descripcion, fecha_registro_inicial, fecha_registro_final, centro_trabajo_id, estado_registro_id);
	...
	var ejecutarCrearEditarRegistroAjax = function(tipo_funcion, id, nombre, descripcion, centro_trabajo_id, estado_registro_id){
		var obj = new Object();
		obj.id = id;
		obj.nombre = nombre;
		obj.descripcion = descripcion;
	...
	var definirBotonVerRegistroVerVentanaModal = function(){
		$( "[id^='reg_div_id_']").click(function() {
		    ...
		    var renglon_centro_trabajo_id = $(this).parent().parent().find("td:nth-child(6)").attr("centro_trabajo_id");
		    var renglon_estado_registro_id = $(this).parent().parent().find("td:nth-child(7)").attr("estado_registro_id");

		    //alert(renglon_estado_registro_id);
			$("#modal_nuevo_edit_duplicar_borrar .modal-title").html("Catálogo de Ubicaciones - Ver Registro");//titulo de la ventana
			...
			$('#se_centro_trabajo_id').val(renglon_centro_trabajo_id); //1 nuevo. inicializamos el valor
			...
			$('#se_centro_trabajo_id').prop("disabled", true);  //valor del centro de trabajo
	...
	var definirBotonDuplicarCascada = function(){
		...
		$( ".btn_duplicar_cascada" ).click(function() {
		    ...
		    var centro_trabajo_id = $(this).parent().parent().find("td:nth-child(6)").attr("centro_trabajo_id");
		   ...
		   if(validarCampos(nombre)){
			    ...
			    ejecutarCrearEditarRegistroAjax("nuevo_registro", null, nombre, descripcion, centro_trabajo_id, estado_registro_id);
			}
		});
	}
	...
	var definirBotonDuplicarActualizar = function(){
		...
		$( ".btn_duplicar_actualizar" ).click(function() {
		    ...
		    //alert(renglon_estado_registro_id);
			$("#modal_nuevo_edit_duplicar_borrar .modal-title").html("Catálogo de  - Dupicar Registro");//titulo de la ventana
			...
			$('#se_centro_trabajo_id').prop("disabled", false);  //valor del centro de trabajo
	...
	var definirBotonEditarMostrarVentanaModal = function(){
		$( ".btn_editar" ).click(function() {
		    ...
		    //alert(renglon_estado_registro_id);
			$("#modal_nuevo_edit_duplicar_borrar .modal-title").html("Catálogo de Ubicaciones - Editar Registro");//titulo de la ventana
...
//boton borrar -----------------------------------------
	var definirBotonBorrarMostrarVentanaModal = function(){	
		...
		$( ".btn_borrar" ).click(function() {
		    ...
		    //alert(renglon_estado_registro_id);
			$("#modal_nuevo_edit_duplicar_borrar .modal-title").html("Catálogo de Ubicaciones - Borrar Registro");//titulo de la ventana
	...
	$( "#btn_buscar_base_datos_todos" ).click(function() {
		...
		ejecutarBuscarRegistrosAjax( null, null, null, null, null, null, null);
	});

	//boton buscar -----------------------------------------	
	$( "#btn_buscar_base_datos" ).click(function() {
					
			$("#modal_buscar .modal-title").html("Catálogo de Ubicaciones - Buscar Registro");//titulo de la ventana
	...
	$( "#btn_mostrar_modal_nuevo_registro" ).click(function() {
			...
			$("#modal_nuevo_edit_duplicar_borrar .modal-title").html("Catálogo de Ubicaciones - Nuevo Registro");//titulo de la ventana
			...
			$('#se_centro_trabajo_id').val(centro_trabajo_id_default); //inicializamos el valor
	...
	$( "#btn_mostrar_modal_borrar_bloque_registro" ).click(function() {
			...
			$("#modal_borrar .modal-title").html("Catálogo de Ubicaciones - Borar Registros Seleccionados");//titulo de la ventana
	...
	$( "#btn_nuevo_ejecutar" ).click(function() {
			...
			var centro_trabajo_id = $('#se_centro_trabajo_id').val(); //1 nuevo
	...
	$( "#btn_editar_ejecutar" ).click(function() {
		   ...
		   if(validarCampos(nombre)){
		   		//realizamos la creacion o modificacion del registro
				ejecutarCrearEditarRegistroAjax("editar_registro", renglon_id, nombre, descripcion, centro_trabajo_id, estado_registro_id);
			}
	...
	$( "#btn_buscar_ejecutar" ).click(function() {
		...
		var centro_trabajo_id = $('#se_buscar_campo_centro_trabajo_id').val().trim(); //1 nuevo
		...
		var cb_buscar_campo_centro_trabajo_is_click = $('#cb_buscar_campo_centro_trabajo').is(":checked");
		...
		if(!cb_buscar_campo_centro_trabajo_is_click){
    	...
    	busqueda_parametros[1] = nombre;
    	busqueda_parametros[2] = descripcion;
    	busqueda_parametros[3] = fecha_registro_inicial;
    	busqueda_parametros[4] = fecha_registro_final;
    	busqueda_parametros[5] = centro_trabajo_id;
    	busqueda_parametros[6] = estado_registro_id; 
	...
	var ajustarEstadoRegistros = function(){
		  ...
		  $("#my_table_sorter tr").each(function(){
		  		...
		  		//verificamos el estado de registro en la columna 
	...
	var initElements = function(){
		...
		$(".my_titulo").css("color", color_distintivo_ubicacion_texto);
		$(".my_titulo").css("background-color", color_distintivo_ubicacion_fondo); //asignamos el color de la barra distintiva en las modales

		//barra distintiva en ventanas -----------------------------
		$(".barra_distintiva_color").css("background-color", color_distintivo_ubicacion_fondo); //asignamos el color de la barra distintiva en las modales
	}
